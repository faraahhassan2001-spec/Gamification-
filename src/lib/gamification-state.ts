...
type State = {
  ...
  /** dealer-confirmed receipt dates for delivered physical rewards */
...
export const actions = {
  ...
  confirmReceipt(id: string) {
    set({ received: { ...state.received, [id]: claimDate } });
  },
  redeem(id: string) {
...
export function useRewardRows() {
  ...
  return baseRewardRows.map((r) => {
    if (s.received[r.id])
      return { ...r, status: "Received" as const, receivedOn: s.received[r.id] };
...
/** Redeem a device: deducts points and records a history entry. */
export function redeemDevice(name: string, points: number) {
  pointsSpent += points;
  extraActivity = [
    {
      id: `pa-dev-${Date.now()}`,
      date: claimDate,
      label: `Device Redemption – ${name}`,
      amount: -points,
      type: "Redeemed",
    },
    ...extraActivity,
