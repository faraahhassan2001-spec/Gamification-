...
export const loyalty = {
  ...
  atRisk: true,
  atRiskHint: "You need 800 more points by 31 Dec 2026 to keep Gold.",
  isTopTier: false,
};
...
export const spinHistory = [
  { id: "s1", date: "18 Aug 2026", outcome: "500 Points", reward: "+500 pts credited" },
...
export const mysteryBoxReward = { label: "500 Points" };
...
export const competitions: Competition[] = [
  ...
  },
  {
    ...
    scorePoints: 50,
    ptc: 750,
    expiryNote: "1 Day to expiry this coopetition .",
    endingIn: "1 Day",
  },
  {
    id: "cmp-daily-streak",
  ...
  },
  {
    ...
    description: "Sell device bundles this month and collect PTC points.",
    rewardKind: "points",
    rewardLabel: "600 PTC",
  ...
  {
    id: "cmp-ptc-boost",
    title: "PTC Boost Week",
    description: "Earn extra PTC on every prepaid activation during the boost week.",
    rewardKind: "points",
    rewardLabel: "270 PTC",
    activity: "Prepaid Activation",
    activityTarget: "2000 Prepaid Activation",
...
/** Lifecycle steps used by the delivery tracker (physical gifts only). */
export const rewardTrackingSteps: RewardStatus[] = [
  "Granted",
  "Claimed",
  "Being Prepared",
  "Shipped",
...
export type RewardRow = {
  ...
  /** deliverable rewards get the tracking timeline */
  physical?: boolean;
  granted: string;
  needsClaim?: boolean;
  /** backend-supplied only */
  claimedOn?: string;
  /** backend-supplied: date the dealer confirmed receipt */
  receivedOn?: string;
  tracking?: string;
  courier?: string;
  pickup?: string;
...
export const rewardRows: RewardRow[] = [
  ...
  },
  {
    ...
    granted: "10 Aug 2026",
    claimedOn: "11 Aug 2026",
    note: "Your reward will be delivered within 3–5 business days.",
  },
  {
    id: "rw3",
  ...
  },
  {
    ...
    pickup: "Main branch — Sun to Thu, 9:00 – 17:00",
...
/** Tone used by the existing StatusChip for each lifecycle status. */
/**
 * Dealer-facing status.
 * Physical rewards follow Granted -> Claimed -> Being Prepared -> Shipped ->
 * Delivered -> Received. Non-physical rewards jump straight to Received once
 * issued/credited.
 */
export function dealerRewardStatus(r: {
  status: RewardStatus;
  physical?: boolean;
}): RewardStatus {
  if (!r.physical) return r.status === "Granted" ? "Granted" : "Received";
  return r.status === "Ready" ? "Shipped" : r.status;
}

export function rewardStatusTone(status: RewardStatus): "green" | "orange" {
  return status === "Claimed" || status === "Delivered" || status === "Received"
    ? "green"
    : "orange";
}
...
/* Loyalty levels grid (design) */
export const pageLevels = [
  { pc: 450, level: 1, name: "Tier name", tone: "from-amber-500 to-amber-700", active: true },
  { pc: 600, level: 2, name: "Tier name", tone: "from-lime-500 to-green-700" },
  { pc: 800, level: 3, name: "Tier name", tone: "from-slate-300 to-slate-500" },
  { pc: 1000, level: 4, name: "Tier name", tone: "from-emerald-400 to-emerald-600" },
  { pc: 1200, level: 5, name: "Tier name", tone: "from-orange-400 to-orange-600" },
  { pc: 1500, level: 6, name: "Tier name", tone: "from-sky-400 to-sky-600" },
  { pc: 2000, level: 7, name: "Tier name", tone: "from-teal-400 to-teal-600" },
  { pc: 2500, level: 8, name: "Tier name", tone: "from-pink-400 to-pink-500" },
  { pc: 5000, level: 9, name: "Tier name", tone: "from-indigo-400 to-violet-600" },
];


/* Wallet Point redeem tabs (design) */
export const redeemTabs = ["Voucher", "Device", "Cash", "Router", "Tablet"] as const;

export const redeemRates: Record<string, { points: number; unit: string; unitLabel: string }> = {
  Voucher: { points: 1000, unit: "1 voucher", unitLabel: "Voucher" },
  Device: { points: 25000, unit: "1 device", unitLabel: "Device" },
  Cash: { points: 100, unit: "1 JOD", unitLabel: "JOD" },
  Router: { points: 8000, unit: "1 router", unitLabel: "Router" },
  Tablet: { points: 20000, unit: "1 tablet", unitLabel: "Tablet" },
};
...
export const pointsActivity: {
  ...
  { id: "pa1", date: "Aug 10, 2026", label: "HBB Activation Challenges", amount: 12, type: "Earned" },
  { id: "pa2", date: "Aug 9, 2026", label: "Voucher Redemption", amount: -1000, type: "Redeemed" },
  { id: "pa3", date: "Aug 8, 2026", label: "Points Converted to Wallet", amount: -3000, type: "Converted", detail: "Received 30 JOD" },
  { id: "pa4", date: "Aug 7, 2026", label: "Marketplace Reward Redemption", amount: -4500, type: "Redeemed" },
  { id: "pa5", date: "Aug 7, 2026", label: "Quick Pay Transaction", amount: 12, type: "Earned" },
...
/* Device redemption catalogue (points -> a specific device) */
export type RedeemDevice = {
  id: string;
  name: string;
  points: number;
  emoji: string;
...
/** Dealer points balance used by the Wallet Point screen. */
export const dealerPointsBalance = 45000;

export const redeemDevices: RedeemDevice[] = [
  { id: "dev-iphone-15-pm", name: "iPhone 15 Pro Max", points: 50000, emoji: "📱", stock: "Available" },
  { id: "dev-galaxy-s24", name: "Samsung Galaxy S24", points: 40000, emoji: "📱", stock: "Available" },
  { id: "dev-airpods-pro", name: "AirPods Pro", points: 15000, emoji: "🎧", stock: "Low stock" },
  { id: "dev-5g-router", name: "5G Router", points: 20000, emoji: "📶", stock: "Available" },
];
