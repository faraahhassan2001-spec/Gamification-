...
import { ArrowLeft, RotateCw, Search, SlidersHorizontal } from "lucide-react";
    ...
    <div className="relative flex items-center gap-3 overflow-hidden rounded-2xl bg-gradient-to-r from-[#152a4e] via-[#1f4f80] to-[#3f9fc4] px-4 py-4 text-white shadow-md">
      ...
      <div className="relative flex-1">
        ...
        <p className="text-[12px] text-white/80">
          Conversion Value: {balance.toFixed(2)} {currency}
        </p>
      </div>
...
export function StatusChip({ children, tone = "orange" }: { children: ReactNode; tone?: "orange" | "green" }) {
  return (
    <span
      className={`rounded-full px-2.5 py-1 text-[11px] font-medium ${
        tone === "green" ? "bg-emerald-50 text-emerald-600" : "bg-orange-50 text-orange-500"
      }`}
    >
      {children}
    </span>
