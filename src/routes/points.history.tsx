      ...
      {list.length === 0 ? (
        ...
        <ul className="space-y-3">
          {list.map((p) => (
            <li
              key={p.id}
              className="flex items-center justify-between gap-3 rounded-2xl bg-card px-4 py-4 shadow-[0_2px_10px_rgba(15,42,80,0.06)]"
            >
              <div className="min-w-0">
                <p className="text-[12px] text-muted-foreground">{p.date}</p>
                <p className="mt-1 text-[15px] font-bold text-foreground">{p.label}</p>
              </div>
              <span
