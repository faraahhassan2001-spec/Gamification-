  ...
  head: () => ({
    meta: [
      { title: "My Challenges — Dealer Gamification" },
      {
        name: "description",
        content:
          "Track a single dealer challenge: target, dates, participants, progress and your spin reward.",
      },
      { property: "og:title", content: "My Challenges — Dealer Gamification" },
      {
        property: "og:description",
        content: "Challenge target, progress and spin reward in one view.",
      },
    ...
    >
      <section className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#3d2f63] via-[#453d78] to-[#4d5b93] p-4 text-white shadow-md">
        ...
        <div className="mt-3 space-y-2 text-[13px] text-white/85">
          ...
          <p className="flex items-center gap-2">
            <Users className="size-4" /> {c.participants} Joined
          </p>
        </div>
        <div className="mt-3 flex flex-wrap gap-2 text-[11px]">
          ...
          <span className="rounded-full bg-white/15 px-3 py-1.5">🏆 {c.ptc}PTC</span>
        </div>
      </section>
      ...
      {c.rewardKind === "mystery" ? (
        ...
        <GamCard>
          <p className="mb-2 text-[13px] text-muted-foreground">Unlock your spin reward 👉</p>
          <SpinWheel />
          <button
            onClick={() => setSpinOpen(true)}
            className="mt-4 w-full rounded-xl bg-sky-50 py-3 text-[14px] font-medium text-primary"
          >
            Open Spin The Wheel
          </button>
        </GamCard>
